...
<style type="text/css">
   ...
   /*------------------diary css 시작------------------------------- */
/* #= id , . = class*/
 #diaryTitle { width:800px; text-align: center; margin-top: 5px}  
 #diaryContent { width: 800px; text-align: center}
 
 .dTable { border:1px solid #E1E6F6; border-spacing: 0px }
 th { width: 120px; border: 1px solid #E1E6F6; }
 .sunTitle{ background-color: #FF0000; font-weight: bold; color: #FFFFFF }
 .setTitle{background-color: #0000FF;  font-weight: bold; color: #FFFFFF }
 #dayTitle{font-size:20px; font-family: 돋움; font-weight: bold; vertical-align: top;}
 td { width: 120px; height: 50px; border: 1px solid #E6E6E7; text-align: left; vertical-align: top;
  padding-left: 3px; padding-top: 2px}
 .blank { background-color: #F1F1F1}
 .sun { color: #FF1049; font-weight: bold;}
 .sat { color: #003CFF; font-weight: bold;}
 .week { color: #333333; }
 
 #toDay { border: 2px solid #0099FF; background-color: #E8F2FE;   }
   /*------------------diary css 끝------------------------------- */
...
////
 ...
 <a href="ch.jsp?param_month=<%=nowMonth-1==0 ? 12:nowMonth-1
 %>&param_year=<%= nowMonth-1 == 0? nowYear-1:nowYear
 %> "><img src="images/prev_btn.png" title="이전월"/></a>
 <span id="dayTitle"> <%= nowYear %>.<%=nowMonth %> </span>
 
 <a href="diary.jsp?param_month=<%=nowMonth+1==13?1:nowMonth+1
 %>&param_year=<%= nowMonth+1==13?nowYear+1:nowYear
 %>"><img src="images/next_btn.png" title="다음월"/></a>
 
 <!-- 파라미터에 값이 없으니까 if를 타지 않고 현재년과 월이 나와용 -->
 <a href="diary.jsp"><img src="images/today_btn.png" title="오늘"/></a>
 </div>
 <div id="diaryContent">
   <table class="dTable">
   <thead>
   ...
   <%
    ...
    ///////////////////////////5.9일 코드추가 끝//////////////
     ...
     %>
     <td <%=todayCss %>> <span class="<%=dayCss%>"> <%= tempDay %> </span> </td> 
